...
} from '../find-sesac-user.js';

import {
    sendEmailVerifyCode
} from "../api/memberApi.js";

export function onSesacUserConfirmModalCloseBtnClick() {
    self.location = "/find-sesac-user";
}

export function onSesacUserNotExistModalCloseBtnClick() {
    self.location = "/find-sesac-user";
}
...
// Sesac 회원 가져오기 클릭 이벤트 핸들러
export function onFindSesacUserFormSubmitBtnClick(event) {
    event.preventDefault();

    // form data
    const findSesacUserForm     = document.querySelector("#findSesacUserForm");
    const emailInput     = document.querySelector("#email");
    const emailCodeInput = document.querySelector("#email-code");
